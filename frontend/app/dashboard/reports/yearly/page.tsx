...
const initialReports: YearlyReport[] = years.map((y, i) => ({
  id: `y${y}`,
  year: y,
  totalOrders: 1500 + i * 120,
  totalRevenue: Math.floor(400000 + Math.random() * 200000),
  profitMargin: Math.floor(20 + Math.random() * 15),
  topProduct: ["Smartphone", "Laptop", "Headphones"][i % 3],
}));
...
function DraggableRow({ row }: { row: Row<YearlyReport> }) {
...
export default function AdminYearlyReport() {
  const [data, setData] = React.useState<YearlyReport[]>(initialReports);
  ...
  // Columns
  const columns = React.useMemo<ColumnDef<YearlyReport>[]>(() => [
    {
      id: "drag",
      header: () => null,
  ...
  const filteredData = React.useMemo(() => {
    return data.filter((d) =>
      d.year.toString().includes(globalFilter.toLowerCase())
    );
  }, [data, globalFilter]);
  ...
  const handleExport = () => {
    const csvRows = [
      ["Year", "Orders", "Revenue", "Profit Margin", "Top Product"],
      ...data.map((r) => [
        r.year,
        r.totalOrders,
        r.totalRevenue,
    ...
    a.click();
    toast.success("Yearly Report exported successfully!");
  };
    ...
    >
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Analyze yearly revenue, orders, and profit performance.
            </p>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-4">
        ...
        <div className="rounded-lg border p-4">
          ...
          <p className="text-xs text-muted-foreground">Yearly records</p>
        </div>
      </div>
  ...
  <div className="rounded-lg border p-4 shadow-sm">
    <h3 className="text-sm font-medium mb-2">Total Orders per Year</h3>
    <ResponsiveContainer width="100%" height={250}>
      <BarChart data={data}>
        <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" />
        <XAxis dataKey="year" tick={{ fontSize: 12 }} />
